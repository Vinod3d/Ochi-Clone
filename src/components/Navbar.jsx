...
const Navbar = () => {
  ...
  return (
    <div className='fixed z-[999] w-full bg-zinc-900 px-6 md:px-20 py-8 flex justify-between items-center'>
      <div className="logo">
      ...
      <div className="hidden lg:flex links gap-10 font1">
        {["Services", "Our Work", "About Us", "Insights", "Contact"].map((item, index) => (
          <a href="#" key={index} className={`text-lg font-light ${index === 4 && "ml-32"}`}>{item}</a>
        ))}
      </div>
      <div className="lg:hidden flex items-center">
        <button onClick={toggleMobileMenu} className="text-3xl z-10">
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed top-0 left-0 w-full h-full bg-zinc-900 flex flex-col items-center justify-center transition-transform transform ${
      ...
      >
        {["Services", "Our Work", "About Us", "Insights", "Contact"].map((item, index) => (
          <a
            href="#"
            key={index}
            className="text-2xl font-light my-4"
